...
export class ScriptService {
  ...
  registerCaptchaScript(
    ...
    // note: https://github.com/niteshpurohit/ngx-captcha/issues/2
    window[this.windowOnLoadCallbackProperty] = <any>(
    ...
    scriptElem.src = this.getCaptchaScriptUrl(
      useGlobalDomain,
      render,
      language
    );
  ...
  private getCaptchaScriptUrl(
    ...
    return `https://www.${domain}/recaptcha/api.js?onload=${
      this.windowOnLoadCallbackProperty
    }&render=${render}${this.getLanguageParam(language)}`;
  }
}
